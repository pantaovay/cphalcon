...
PHP_METHOD(Phalcon_Validation, __construct) {
	...
	if (zephir_fast_count_int(&validators TSRMLS_CC)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "array_filter", NULL, 422, &validators, &_0$$3);
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_filter", NULL, 422, &validators, &_2$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, appendMessage) {
	...
	ZEPHIR_CALL_METHOD(NULL, &messages, "appendmessage", NULL, 423, message);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, SL("_messages"), &messages);
	RETURN_THIS();
			...
			if (Z_TYPE_P(&entity) == IS_OBJECT) {
				...
				if ((zephir_method_exists(&entity, &method TSRMLS_CC)  == SUCCESS)) {
					...
					if ((zephir_method_exists_ex(&entity, SL("writeattribute") TSRMLS_CC) == SUCCESS)) {
						...
						ZEPHIR_CALL_FUNCTION(&_4$$28, "property_exists", NULL, 302, &entity, &field);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, preChecking) {
	...
	zval _9$$13;
	zend_bool _5$$10, _6$$10, _7$$7;
	zephir_fcall_cache_entry *_1 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *field, field_sub, *validator, validator_sub, __$true, singleField, allowEmpty, emptyValue, value, result, *_0$$3, _2$$6, _3$$6, *_4$$9, raw$$7, _8$$13, _10$$13, _11$$13;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(field), _0$$3)
		{
			ZEPHIR_INIT_NVAR(&singleField);
			ZVAL_COPY(&singleField, _0$$3);
			ZEPHIR_CALL_METHOD(&result, this_ptr, "prechecking", &_1, 424, &singleField, validator);
			zephir_check_call_status();
		...
		if (zephir_is_true(&allowEmpty)) {
			...
			if (Z_TYPE_P(&allowEmpty) == IS_ARRAY) {
				...
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&allowEmpty), _4$$9)
				{
					...
					_5$$10 = Z_TYPE_P(&emptyValue) == IS_OBJECT;
					if (_5$$10) {
						_5$$10 = Z_TYPE_P(&value) == IS_OBJECT;
					}
					_6$$10 = _5$$10;
					if (_6$$10) {
						_6$$10 = ZEPHIR_IS_EQUAL(&emptyValue, &value);
					}
			...
			_7$$7 = Z_TYPE_P(&value) == IS_OBJECT;
			if (_7$$7) {
				_7$$7 = (zephir_method_exists_ex(&value, SL("__tostring") TSRMLS_CC) == SUCCESS);
			}
			if (UNEXPECTED(_7$$7)) {
				ZEPHIR_CALL_METHOD(&_8$$13, &value, "__tostring", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_INIT_VAR(&raw$$7);
				zephir_fast_strtolower(&raw$$7, &_8$$13);
				ZEPHIR_INIT_VAR(&_9$$13);
				zephir_create_array(&_9$$13, 3, 0 TSRMLS_CC);
				ZEPHIR_INIT_VAR(&_10$$13);
				ZVAL_STRING(&_10$$13, "null");
				zephir_array_fast_append(&_9$$13, &_10$$13);
				ZEPHIR_INIT_NVAR(&_10$$13);
				ZVAL_STRING(&_10$$13, "false");
				zephir_array_fast_append(&_9$$13, &_10$$13);
				ZEPHIR_INIT_NVAR(&_10$$13);
				ZVAL_STRING(&_10$$13, "");
				zephir_array_fast_append(&_9$$13, &_10$$13);
				ZEPHIR_CALL_FUNCTION(&_11$$13, "in_array", NULL, 353, &raw$$7, &_9$$13, &__$true);
				zephir_check_call_status();
