...
PHP_METHOD(Phalcon_Mvc_Router, handle) {
	...
	zephir_is_iterable(&_4, 0, "phalcon/mvc/router.zep", 578);
	ZEND_HASH_REVERSE_FOREACH_VAL(Z_ARRVAL_P(&_4), _5)
	{
		...
		if (zephir_is_true(&routeFound)) {
			...
			if (Z_TYPE_P(&matches) == IS_ARRAY) {
				...
				zephir_check_call_status();
				zephir_is_iterable(&paths, 0, "phalcon/mvc/router.zep", 567);
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&paths), _23$$34, _24$$34, _22$$34)
				{
					...
					if (Z_TYPE_P(&part) != IS_STRING) {
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_25$$36, "phalcon/mvc/router.zep", 520 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
...
PHP_METHOD(Phalcon_Mvc_Router, mount) {
	...
	zephir_check_call_status();
	if (Z_TYPE_P(&beforeMatch) != IS_NULL) {
		zephir_is_iterable(&groupRoutes, 0, "phalcon/mvc/router.zep", 810);
	...
	zephir_check_call_status();
	if (Z_TYPE_P(&hostname) != IS_NULL) {
		zephir_is_iterable(&groupRoutes, 0, "phalcon/mvc/router.zep", 819);
...
PHP_METHOD(Phalcon_Mvc_Router, getRouteById) {
	...
	zephir_is_iterable(&_0, 0, "phalcon/mvc/router.zep", 937);
...
PHP_METHOD(Phalcon_Mvc_Router, getRouteByName) {
	...
	zephir_read_property(&_0, this_ptr, SL("_routes"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "phalcon/mvc/router.zep", 952);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _1)
	{
		ZEPHIR_INIT_NVAR(&route);
		ZVAL_COPY(&route, _1);
