...
PHP_METHOD(Phalcon_Cache_Backend_Redis, __construct) {
	...
	zval *frontend, frontend_sub, *options = NULL, options_sub, __$false, __$null, _1, _3, _4$$4, _5$$5, _6$$6, _7$$7, _8$$9;
	ZEPHIR_INIT_THIS();
	...
	if (_0) {
		zephir_array_fetch_string(&_1, options, SL("redis"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/redis.zep", 75 TSRMLS_CC);
		_0 = !(ZEPHIR_IS_EMPTY(&_1));
	}
	_2 = _0;
	if (_2) {
		ZEPHIR_OBS_VAR(&_3);
		zephir_array_fetch_string(&_3, options, SL("redis"), PH_NOISY, "phalcon/cache/backend/redis.zep", 75 TSRMLS_CC);
		_2 = Z_TYPE_P(&_3) == IS_OBJECT;
	}
	if (_2) {
		zephir_array_fetch_string(&_4$$4, options, SL("redis"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/redis.zep", 76 TSRMLS_CC);
	...
	ZEPHIR_CALL_PARENT(NULL, phalcon_cache_backend_redis_ce, this_ptr, "__construct", &_9, 113, frontend, options);
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, _connect) {
	...
	if (zephir_array_isset_string_fetch(&index, &options, SL("index"), 0)) {
		...
		zephir_check_call_status();
		if (!(zephir_is_true(&success))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Redisd server selected database failed", "phalcon/cache/backend/redis.zep", 139);
			return;
		}
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, queryKeys) {
	...
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCR')!", "phalcon/cache/backend/redis.zep", 330);
		return;
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Redis, flush) {
	...
	if (ZEPHIR_IS_STRING(&specialKey, "")) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_cache_exception_ce, "Cached keys need to be enabled to use this function (options['statsKey'] == '_PHCR')!", "phalcon/cache/backend/redis.zep", 469);
		return;
	}
	ZEPHIR_CALL_METHOD(&keys, &redis, "smembers", NULL, 0, &specialKey);
	zephir_check_call_status();
