...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addInherit) {
	...
	if (zephir_array_isset(&_2, &roleInheritName)) {
		...
		zephir_is_iterable(&_4$$6, 0, "phalcon/acl/adapter/memory.zep", 232);
	...
	if (!(zephir_array_isset(&rolesNames, &roleInheritName))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_7$$8, "phalcon/acl/adapter/memory.zep", 238 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, addResourceAccess) {
	...
	if (Z_TYPE_P(accessList) == IS_ARRAY) {
		zephir_is_iterable(accessList, 0, "phalcon/acl/adapter/memory.zep", 335);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, dropResourceAccess) {
	...
	if (Z_TYPE_P(accessList) == IS_ARRAY) {
		zephir_is_iterable(accessList, 0, "phalcon/acl/adapter/memory.zep", 361);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, _allowOrDeny) {
	...
	if (Z_TYPE_P(access) == IS_ARRAY) {
		...
		ZEPHIR_INIT_NVAR(&accessName);
		zephir_is_iterable(access, 0, "phalcon/acl/adapter/memory.zep", 406);
		...
		if (!ZEPHIR_IS_STRING(access, "*")) {
			...
			if (!(zephir_array_isset(&accessList, &accessKey))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_10$$12, "phalcon/acl/adapter/memory.zep", 411 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, allow) {
	...
	if (!ZEPHIR_IS_STRING(&roleName, "*")) {
		...
		zephir_is_iterable(&_2$$4, 0, "phalcon/acl/adapter/memory.zep", 458);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, deny) {
	...
	if (!ZEPHIR_IS_STRING(&roleName, "*")) {
		...
		zephir_is_iterable(&_1$$4, 0, "phalcon/acl/adapter/memory.zep", 491);
...
PHP_METHOD(Phalcon_Acl_Adapter_Memory, isAllowed) {
	...
	if (Z_TYPE_P(&haveAccess) == IS_NULL) {
		...
		if (zephir_array_isset_fetch(&inheritedRoles, &roleInherits, roleName, 0 TSRMLS_CC)) {
			if (Z_TYPE_P(&inheritedRoles) == IS_ARRAY) {
				zephir_is_iterable(&inheritedRoles, 0, "phalcon/acl/adapter/memory.zep", 580);
	...
	if (Z_TYPE_P(&haveAccess) == IS_NULL) {
		...
		if (zephir_array_isset(&accessList, &accessKey)) {
			...
			if (Z_TYPE_P(&inheritedRoles) == IS_ARRAY) {
				zephir_is_iterable(&inheritedRoles, 0, "phalcon/acl/adapter/memory.zep", 610);
	...
	if (Z_TYPE_P(&funcAccess) != IS_NULL) {
		...
		do {
			...
			zephir_is_iterable(&reflectionParameters$$30, 0, "phalcon/acl/adapter/memory.zep", 699);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&reflectionParameters$$30), _11$$32)
			{
				...
				if (Z_TYPE_P(&reflectionClass$$30) != IS_NULL) {
					...
					if (_21$$34) {
						...
						zephir_array_fetch(&_22$$34, &parameters, &parameterToCheck$$30, PH_NOISY, "phalcon/acl/adapter/memory.zep", 690 TSRMLS_CC);
						_21$$34 = Z_TYPE_P(&_22$$34) == IS_OBJECT;
					}
					_23$$34 = _21$$34;
					if (_23$$34) {
						zephir_array_fetch(&_25$$34, &parameters, &parameterToCheck$$30, PH_NOISY | PH_READONLY, "phalcon/acl/adapter/memory.zep", 690 TSRMLS_CC);
					...
					if (_14$$34) {
						...
						zephir_array_fetch(&_29$$37, &parameters, &parameterToCheck$$30, PH_NOISY | PH_READONLY, "phalcon/acl/adapter/memory.zep", 691 TSRMLS_CC);
						...
						zephir_check_call_status();
						zephir_throw_exception_debug(&_27$$37, "phalcon/acl/adapter/memory.zep", 691 TSRMLS_CC);
						ZEPHIR_MM_RESTORE();
						return;
					}
				}
				...
				if (_34$$33) {
					zephir_array_fetch(&_35$$38, &parameters, &parameterToCheck$$30, PH_NOISY | PH_READONLY, "phalcon/acl/adapter/memory.zep", 696 TSRMLS_CC);
					zephir_array_append(&parametersForFunction$$30, &_35$$38, PH_SEPARATE, "phalcon/acl/adapter/memory.zep", 696);
				}
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&reflectionParameter$$30);
			...
			if (zephir_fast_count_int(&parametersForFunction$$30 TSRMLS_CC) == 0) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_46$$43, "phalcon/acl/adapter/memory.zep", 717 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		} while(0);
