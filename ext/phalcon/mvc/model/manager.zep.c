...
PHP_METHOD(Phalcon_Mvc_Model_Manager, isVisibleModelProperty) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval property;
	zval *model, model_sub, *property_param = NULL, properties, modelVisibility, className, _0$$3;
	ZEPHIR_INIT_THIS();

	ZVAL_UNDEF(&model_sub);
	ZVAL_UNDEF(&properties);
	ZVAL_UNDEF(&modelVisibility);
	ZVAL_UNDEF(&className);
	ZVAL_UNDEF(&_0$$3);
	ZVAL_UNDEF(&property);
	...
	zephir_read_property(&modelVisibility, this_ptr, SL("_modelVisibility"), PH_NOISY_CC);
	ZEPHIR_INIT_VAR(&className);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, notifyEvent) {
	...
	if (Z_TYPE_P(&behaviors) == IS_ARRAY) {
		...
		if (zephir_array_isset_fetch(&modelsBehaviors, &behaviors, &_0$$3, 0 TSRMLS_CC)) {
			zephir_is_iterable(&modelsBehaviors, 0, "phalcon/mvc/model/manager.zep", 526);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, missingMethod) {
	...
	if (Z_TYPE_P(&behaviors) == IS_ARRAY) {
		...
		zephir_get_class(&_0$$3, model, 1 TSRMLS_CC);
		if (zephir_array_isset_fetch(&modelsBehaviors, &behaviors, &_0$$3, 0 TSRMLS_CC)) {
			zephir_is_iterable(&modelsBehaviors, 0, "phalcon/mvc/model/manager.zep", 582);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&modelsBehaviors), _1$$4)
			{
				ZEPHIR_INIT_NVAR(&behavior);
				ZVAL_COPY(&behavior, _1$$4);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, _mergeFindParameters) {
	...
	if (Z_TYPE_P(findParamsOne) == IS_ARRAY) {
		zephir_is_iterable(findParamsOne, 0, "phalcon/mvc/model/manager.zep", 1176);
	...
	if (Z_TYPE_P(findParamsTwo) == IS_ARRAY) {
		zephir_is_iterable(findParamsTwo, 0, "phalcon/mvc/model/manager.zep", 1210);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, getRelationRecords) {
	...
	if (zephir_is_true(&_0)) {
		...
		ZEPHIR_CALL_METHOD(&_5$$3, this_ptr, "_mergefindparameters", &_6, 322, &extraParameters, parameters);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(&fields) != IS_ARRAY) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_16$$10, 0, "phalcon/mvc/model/manager.zep", 1330);
	...
	ZEPHIR_CALL_METHOD(&findArguments, this_ptr, "_mergefindparameters", &_6, 322, &findParams, parameters);
	zephir_check_call_status();
	if (Z_TYPE_P(&extraParameters) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&findParams, this_ptr, "_mergefindparameters", &_6, 322, &findArguments, &extraParameters);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, getRelations) {
	...
	if (Z_TYPE_P(&belongsTo) == IS_ARRAY) {
		...
		if (zephir_array_isset_fetch(&relations, &belongsTo, &entityName, 0 TSRMLS_CC)) {
			zephir_is_iterable(&relations, 0, "phalcon/mvc/model/manager.zep", 1613);
	...
	if (Z_TYPE_P(&hasMany) == IS_ARRAY) {
		...
		if (zephir_array_isset_fetch(&relations, &hasMany, &entityName, 0 TSRMLS_CC)) {
			zephir_is_iterable(&relations, 0, "phalcon/mvc/model/manager.zep", 1625);
...
PHP_METHOD(Phalcon_Mvc_Model_Manager, __destruct) {
	...
	ZEPHIR_CALL_CE_STATIC(NULL, phalcon_mvc_model_query_ce, "clean", &_0, 323);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
