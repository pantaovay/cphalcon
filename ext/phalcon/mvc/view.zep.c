...
PHP_METHOD(Phalcon_Mvc_View, setViewsDir) {
	...
	if (Z_TYPE_P(viewsDir) == IS_STRING) {
		...
		zephir_is_iterable(viewsDir, 0, "phalcon/mvc/view.zep", 210);
...
PHP_METHOD(Phalcon_Mvc_View, _loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (Z_TYPE_P(&registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 376, this_ptr, &dependencyInjector);
			zephir_check_call_status();
			...
			zephir_is_iterable(&registeredEngines, 0, "phalcon/mvc/view.zep", 565);
...
PHP_METHOD(Phalcon_Mvc_View, _engineRender) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_1, 0, "phalcon/mvc/view.zep", 694);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_1), _2)
	{
		ZEPHIR_INIT_NVAR(&viewsDir);
		ZVAL_COPY(&viewsDir, _2);
		ZEPHIR_CALL_METHOD(&_3$$3, this_ptr, "_isabsolutepath", &_4, 377, &viewPath);
		zephir_check_call_status();
		...
		zephir_is_iterable(engines, 0, "phalcon/mvc/view.zep", 692);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(engines), _13$$3, _14$$3, _12$$3)
		{
			...
			if ((zephir_file_exists(&viewEnginePath TSRMLS_CC) == SUCCESS)) {
				if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
					...
					zephir_create_array(&_15$$17, 1, 0 TSRMLS_CC);
					zephir_array_fast_append(&_15$$17, &viewEnginePath);
					zephir_update_property_zval(this_ptr, SL("_activeRenderPaths"), &_15$$17);
					ZEPHIR_INIT_NVAR(&_17$$17);
					ZVAL_STRING(&_17$$17, "view:beforeRenderView");
					ZEPHIR_CALL_METHOD(&_16$$17, &eventsManager, "fire", &_18, 0, &_17$$17, this_ptr, &viewEnginePath);
					zephir_check_call_status();
					if (ZEPHIR_IS_FALSE_IDENTICAL(&_16$$17)) {
						continue;
					}
				}
				if (mustClean) {
				...
				ZEPHIR_CALL_METHOD(NULL, &engine, "render", NULL, 0, &viewEnginePath, &viewParams, &_19$$16);
				zephir_check_call_status();
				notExists = 0;
				if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
					ZEPHIR_INIT_NVAR(&_20$$19);
					ZVAL_STRING(&_20$$19, "view:afterRenderView");
					ZEPHIR_CALL_METHOD(NULL, &eventsManager, "fire", &_21, 0, &_20$$19, this_ptr);
					zephir_check_call_status();
				}
				break;
			}
	...
	if (notExists == 1) {
		if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
			...
			ZVAL_STRING(&_22$$21, "view:notFoundView");
			ZEPHIR_CALL_METHOD(NULL, &eventsManager, "fire", NULL, 0, &_22$$21, this_ptr, &viewEnginePath);
			zephir_check_call_status();
		}
		if (!(silence)) {
			ZEPHIR_INIT_VAR(&_23$$22);
			object_init_ex(&_23$$22, phalcon_mvc_view_exception_ce);
			ZEPHIR_INIT_VAR(&_24$$22);
			ZEPHIR_CONCAT_SVS(&_24$$22, "View '", &viewPath, "' was not found in any of the views directory");
			ZEPHIR_CALL_METHOD(NULL, &_23$$22, "__construct", NULL, 9, &_24$$22);
			zephir_check_call_status();
			zephir_throw_exception_debug(&_23$$22, "phalcon/mvc/view.zep", 704 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
		...
		if (renderLevel >= 2) {
			if (!(zephir_array_isset_long(&disabledLevels, 2))) {
				...
				if (Z_TYPE_P(&templatesBefore) == IS_ARRAY) {
					silence = 0;
					zephir_is_iterable(&templatesBefore, 0, "phalcon/mvc/view.zep", 903);
		...
		if (renderLevel >= 4) {
			if (!(zephir_array_isset_long(&disabledLevels, 4))) {
				...
				if (Z_TYPE_P(&templatesAfter) == IS_ARRAY) {
					silence = 0;
					zephir_is_iterable(&templatesAfter, 0, "phalcon/mvc/view.zep", 934);
...
PHP_METHOD(Phalcon_Mvc_View, cache) {
	...
	if (Z_TYPE_P(options) == IS_ARRAY) {
		...
		zephir_is_iterable(options, 0, "phalcon/mvc/view.zep", 1252);
