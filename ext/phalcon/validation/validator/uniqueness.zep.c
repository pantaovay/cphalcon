...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, validate) {
	...
	if (!(zephir_is_true(&_0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 450, &_4$$3, field, &_6$$3, &_5$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation_Validator_Uniqueness, isUniqueness) {
	...
	zephir_check_call_status();
	zephir_is_iterable(field, 0, "phalcon/validation/validator/uniqueness.zep", 128);
	...
	zephir_is_iterable(field, 0, "phalcon/validation/validator/uniqueness.zep", 203);
		...
		if (zephir_is_true(&except)) {
			...
			if (Z_TYPE_P(&fieldExcept) != IS_NULL) {
				if (Z_TYPE_P(&fieldExcept) == IS_ARRAY) {
					zephir_is_iterable(&fieldExcept, 0, "phalcon/validation/validator/uniqueness.zep", 176);
					ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&fieldExcept), _16$$16)
					{
						...
						zephir_array_append(&notInValues, &_18$$17, PH_SEPARATE, "phalcon/validation/validator/uniqueness.zep", 172);
					...
					zephir_array_append(&exceptConditions, &_20$$16, PH_SEPARATE, "phalcon/validation/validator/uniqueness.zep", 176);
				} else {
					ZEPHIR_SINIT_NVAR(_21$$18);
					ZVAL_LONG(&_21$$18, index);
					ZEPHIR_INIT_LNVAR(_22$$18);
					ZEPHIR_CONCAT_VSV(&_22$$18, &attribute, " <> ?", &_21$$18);
					zephir_array_append(&exceptConditions, &_22$$18, PH_SEPARATE, "phalcon/validation/validator/uniqueness.zep", 179);
					zephir_array_update_multi(&params, &fieldExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
					index++;
				}
			} else if (_15$$12) {
				zephir_is_iterable(&except, 0, "phalcon/validation/validator/uniqueness.zep", 190);
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&except), _23$$19)
				{
					...
					zephir_array_append(&notInValues, &_25$$20, PH_SEPARATE, "phalcon/validation/validator/uniqueness.zep", 186);
					zephir_array_update_multi(&params, &singleExcept TSRMLS_CC, SL("sa"), 3, SL("bind"));
					index++;
				} ZEND_HASH_FOREACH_END();
				ZEPHIR_INIT_NVAR(&singleExcept);
	...
	if (ZEPHIR_IS_LONG(&_30, 0)) {
		...
		zephir_check_call_status();
		zephir_is_iterable(&_33$$22, 0, "phalcon/validation/validator/uniqueness.zep", 211);
	...
	ZEPHIR_INIT_NVAR(&_0);
	zephir_array_fetch_string(&_41, &params, SL("conditions"), PH_NOISY | PH_READONLY, "phalcon/validation/validator/uniqueness.zep", 217 TSRMLS_CC);
