...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, inWhere) {
	...
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/mvc/model/criteria.zep", 469);
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, notInWhere) {
	...
	ZEPHIR_INIT_VAR(&bindKeys);
	array_init(&bindKeys);
	zephir_is_iterable(&values, 0, "phalcon/mvc/model/criteria.zep", 506);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&values), _0)
	{
		...
		zephir_array_append(&bindKeys, &_1$$3, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 496);
		zephir_array_update_zval(&bindParams, &key, &value, PH_COPY | PH_SEPARATE);
		ZEPHIR_SEPARATE(&hiddenParam);
		zephir_increment(&hiddenParam);
	} ZEND_HASH_FOREACH_END();
...
PHP_METHOD(Phalcon_Mvc_Model_Criteria, fromInput) {
	...
	if (zephir_fast_count_int(&data TSRMLS_CC)) {
		...
		array_init(&bind);
		zephir_is_iterable(&data, 0, "phalcon/mvc/model/criteria.zep", 750);
			...
			if (zephir_array_isset_fetch(&type, &dataTypes, &attribute, 0 TSRMLS_CC)) {
				...
				if (_8$$7) {
					if (ZEPHIR_IS_LONG(&type, 2)) {
						...
						zephir_array_append(&conditions, &_9$$9, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 739);
					...
					zephir_array_append(&conditions, &_11$$8, PH_SEPARATE, "phalcon/mvc/model/criteria.zep", 746);
	...
	if (zephir_fast_count_int(&conditions TSRMLS_CC)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &criteria, "where", NULL, 318, &_12$$10);
		zephir_check_call_status();
