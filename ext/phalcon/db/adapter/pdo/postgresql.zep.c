...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Postgresql, describeColumns) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "phalcon/db/adapter/pdo/postgresql.zep", 328);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _4)
	{
		...
		zephir_array_fetch_long(&_31$$3, &field, 6, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 292 TSRMLS_CC);
		if (ZEPHIR_IS_STRING(&_31$$3, "PRI")) {
			zephir_array_update_string(&definition, SL("primary"), &__$true, PH_COPY | PH_SEPARATE);
		}
		zephir_array_fetch_long(&_32$$3, &field, 5, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 299 TSRMLS_CC);
		if (ZEPHIR_IS_STRING(&_32$$3, "NO")) {
			zephir_array_update_string(&definition, SL("notNull"), &__$true, PH_COPY | PH_SEPARATE);
		}
		zephir_array_fetch_long(&_33$$3, &field, 7, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 306 TSRMLS_CC);
		...
		zephir_array_fetch_long(&_34$$3, &field, 9, PH_NOISY, "phalcon/db/adapter/pdo/postgresql.zep", 313 TSRMLS_CC);
		if (Z_TYPE_P(&_34$$3) != IS_NULL) {
			zephir_array_fetch_long(&_35$$26, &field, 9, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 314 TSRMLS_CC);
			...
			zephir_array_fetch_string(&_40$$26, &definition, SL("default"), PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 315 TSRMLS_CC);
			ZEPHIR_INIT_NVAR(&_36$$26);
		...
		zephir_check_call_status();
		zephir_array_append(&columns, &_43$$3, PH_SEPARATE, "phalcon/db/adapter/pdo/postgresql.zep", 324);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Postgresql, createTable) {
	...
	if (zephir_fast_count_int(&queries TSRMLS_CC) > 1) {
		...
		/* try_start_1: */
			...
			zephir_is_iterable(&queries, 0, "phalcon/db/adapter/pdo/postgresql.zep", 359);
		...
		zephir_array_fetch_long(&_4$$10, &queries, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 366 TSRMLS_CC);
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo_Postgresql, modifyColumn) {
	...
	if (zephir_fast_count_int(&queries TSRMLS_CC) > 1) {
		...
		/* try_start_1: */
			...
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "begin", NULL, 0);
			zephir_check_call_status_or_jump(try_end_1);
			zephir_is_iterable(&queries, 0, "phalcon/db/adapter/pdo/postgresql.zep", 391);
		...
		if (!(ZEPHIR_IS_EMPTY(&sql))) {
			zephir_array_fetch_long(&_5$$8, &queries, 0, PH_NOISY | PH_READONLY, "phalcon/db/adapter/pdo/postgresql.zep", 400 TSRMLS_CC);
