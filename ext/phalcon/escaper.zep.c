...
PHP_METHOD(Phalcon_Escaper, detectEncoding) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _2)
	{
		...
		ZEPHIR_CALL_FUNCTION(&_3$$5, "mb_detect_encoding", &_4, 177, &str, &charset, &__$true);
	...
	ZEPHIR_INIT_NVAR(&charset);
	ZEPHIR_RETURN_CALL_FUNCTION("mb_detect_encoding", &_4, 177, &str);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Escaper, escapeHtml) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("htmlspecialchars", NULL, 180, &text, &_0, &_1, &_2);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Escaper, escapeHtmlAttr) {
	...
	ZVAL_LONG(&_2, 3);
	ZEPHIR_RETURN_CALL_FUNCTION("htmlspecialchars", NULL, 180, &attribute, &_2, &_0, &_1);
	zephir_check_call_status();
	RETURN_MM();
